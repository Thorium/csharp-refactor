...
/// learns about commands from a compiled command table
/// (CSharpRefactor.vsct -> .cto) that a registered package owns, so this
/// file is the one piece of shell plumbing here.
///
/// These commands drive the `csharp-refactor` global tool, which the
/// analyzer never touches — the analyzer runs in-process in Roslyn, the
/// tool is a separate program that rewrites files. Its output goes to an
/// Output pane rather than a terminal: Visual Studio has no integrated
/// terminal, and a pane is the better home anyway — it persists, it is
/// searchable, and it does not take the keyboard.
namespace CSharp.Refactor.Vsix
...
/// A line to %TEMP%\CSharpRefactor.Vsix.log: a package that loads cleanly
/// and one that registers nothing look identical from outside, so the
/// loader says what it did.
        ...
        lazy
            (let temp = Environment.GetEnvironmentVariable "TEMP"
             ...
             if String.IsNullOrEmpty temp then
                 ...
                 Some(Path.Combine(temp, "CSharpRefactor.Vsix.log")))
...
module internal Commands =
    ...
    [<Literal>]
    let PackageGuidString = "9f175e1b-4a1c-438b-b00d-0d2809bf93d1"

    [<Literal>]
    let CommandSetGuidString = "6e36b499-8953-4fad-a6b5-356af8a17137"

    /// Must match the IDSymbol values in CSharpRefactor.vsct.
    module Ids =
        [<Literal>]
        let Run = 0x0100
    ...
    [<Literal>]
    let RepositoryUrl = "https://github.com/Thorium/csharp-refactor"

    [<Literal>]
    let Tool = "csharp-refactor"

    /// One pane for the life of the session, named so it is findable in the
    /// Output window's drop-down.
    let private paneGuid = Guid "5bb8d7f5-b1a6-4979-8790-e10fbf364aa3"
        ...
        async {
            if isNull pane then
                ...
                | :? IVsOutputWindow as window ->
                    ...
                    window.CreatePane(&id, "CSharp.Refactor", 1, 0) |> ignore
    ...
    let private saveDirtyDocuments () =
        ...
        try
            ...
            | :? IVsSolution as solution ->
                ...
                |> ignore
            | _ -> ()
        with e -> // a failed save must not stop the run
            Trace.write $"package: could not save open documents - {e.Message}"
    ...
    let private runTool (arguments: string) (workingDirectory: string) =
        async {
            ...
            with e ->
                ...
                writeLine
                    $"install it with:  dotnet tool install -g {Tool}   (the hints and light bulbs work without it)"
    ...
    let private onSolution (package: AsyncPackage) (build: string -> string option) =
        ...
        ThreadHelper.JoinableTaskFactory.RunAsync(fun () ->
            task {
                ...
                | None -> box' "CSharp.Refactor" "Open a solution first - the tool runs on a solution or a project."
                | Some(dir, file) ->
    ...
    let runCommand (package: AsyncPackage) =
        onSolution package (fun file ->
            match
                askApply
                    "Run csharp-refactor on this solution?\r\n\r\nIf this repository's .editorconfig sets csharp_refactor.api_changes, the run rewrites the public surface and call sites too, exactly as the --api-changes command does."
            with
            | ValueNone -> None
    ...
    let runApiChangesCommand (package: AsyncPackage) =
        onSolution package (fun file ->
            let question =
                "Run with --api-changes?\r\n\r\nThis also rewrites call sites across the solution's projects, and widens the rules that change a declaration's compiled shape to public declarations."
    ...
    let reportCommand (package: AsyncPackage) =
        onSolution package (fun file ->
            let out = Path.Combine(Path.GetDirectoryName file, "csharp-refactor.sarif")
    ...
    let reviewNotesCommand (package: AsyncPackage) =
        ...
        ThreadHelper.JoinableTaskFactory.RunAsync(fun () ->
            task {
                ...
                match! solutionFile (package :> IAsyncServiceProvider) |> Async.StartAsTask with
                | None -> box' "CSharp.Refactor" "Open a solution first."
                | Some(dir, file) ->
                    let out = Path.Combine(dir, "csharp-refactor-notes.html")
                    showPane ()
    ...
    /// The configuration is a block in `.editorconfig` at the solution's
    /// root: `--create-config` writes it, every value at this build's
    /// default, and never touches a key already there; then the file opens.
    ...
    let statusCommand (package: AsyncPackage) =
        ...
        ThreadHelper.JoinableTaskFactory.RunAsync(fun () ->
            task {
                ...
                let analyzers = Path.Combine(here, "CSharp.Refactor.Analyzers.dll")
                let fsharpCore = Path.Combine(here, "FSharp.Core.dll")
                let temp = Environment.GetEnvironmentVariable "TEMP"
                ...
                let log =
                    if String.IsNullOrEmpty temp then
                        ...
                        Path.Combine(temp, "CSharpRefactor.Vsix.log")
                ...
                writeLine ""
                writeLine $"CSharp.Refactor extension %s{version}"
                writeLine $"  installed at  %s{here}"
                writeLine (sprintf "  analyzers     %s" (present analyzers (File.Exists analyzers)))
                writeLine (sprintf "  FSharp.Core   %s" (present fsharpCore (File.Exists fsharpCore)))
                writeLine $"  apply tool    %s{tool}"
                writeLine $"  package log   %s{log}"

                writeLine
                    "  the hints come from the analyzer asset Roslyn loads into every C# project: no wiring to check"
            }
            :> Task)
        |> ignore
    ...
    let aboutCommand () =
        ...
        let text =
            sprintf
                "CSharp.Refactor %s\r\n\r\nRefactoring hints and one-click quick fixes for C#.\r\n\r\nApply tool: %s\r\n\r\n%s"
                version
                tool
                RepositoryUrl

        box' "About CSharp.Refactor" text

// ---- the package -----------------------------------------------------------

/// Registered through CSharp.Refactor.Vsix.pkgdef, written by hand to
/// agree with these attributes (see the pkgdef's header for why).
///
...
type CSharpRefactorPackage() =
    ...
    override this.InitializeAsync(cancellationToken: CancellationToken, _progress: IProgress<ServiceProgressData>) =
        ...
        task {
            Trace.write "package: InitializeAsync entered"
            ...
            try
                ...
                | :? OleMenuCommandService as commands ->
                    ...
                    Trace.write $"package: {added} commands registered for command set {commandSet}"
                | other ->
                    ...
                    Trace.write $"package: NO commands registered - IMenuCommandService came back as {got}"
            with e ->
                Trace.write $"package: InitializeAsync FAILED - {e.GetType().FullName}: {e.Message}"
